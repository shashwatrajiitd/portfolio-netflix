...
function scrollToSection(sectionId) {
    ...
    if (typeof HistoryManager !== 'undefined' && !HistoryManager.isNavigating) {
        const currentProfile = 'Adventurer';
...
function initializeProfileSelector() {
    ...
    const currentPage = document.getElementById('adventurer-page');
    if (currentPage) {
        updateCurrentProfileImage('Adventurer');
    }
...
function initializeAdventurerPage() {
    ...
    const adventurerNavbar = document.getElementById('adventurer-navbar');
    if (adventurerNavbar) {
        window.addEventListener('scroll', () => {
            // Check if scrolled past threshold
            if (window.scrollY > 50) {
                adventurerNavbar.classList.add('scrolled');
                adventurerNavbar.classList.add('scrolling-up');
            } else {
                adventurerNavbar.classList.remove('scrolled');
                adventurerNavbar.classList.remove('scrolling-up');
            }
        });
    }
    ...
    const logo = document.querySelector('#adventurer-navbar .logo');
    if (logo) {
    ...
    const adventurerPage = document.getElementById('adventurer-page');
    if (adventurerPage) {
        const adventurerLinks = adventurerPage.querySelectorAll('a[href^="#"]');
        adventurerLinks.forEach(link => {
    ...
    initializeVideoCarousel('adventurer_profile');
}
